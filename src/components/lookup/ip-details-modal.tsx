...
import { Building2, Compass, Globe2, MapPin, Network, Server, Wifi } from 'lucide-react';
...
const DetailsView: FC<{ details: IpDetails }> = ({ details }) => {
	const rows: { icon: ReactNode; label: string; value: ReactNode }[] = [
		...
		},
		{
			...
			label: 'Location',
			value: [details.country, details.regionName, details.city].filter(Boolean).join(', '),
		},
		{
		...
		<dl className="space-y-2 text-sm">
			{rows.map(row => (
				...
				>
					...
					<dd className="text-foreground text-right text-sm break-words">{row.value}</dd>
				</div>
			))}
		</dl>
	);

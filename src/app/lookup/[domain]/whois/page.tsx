...
async function load(domain: string): Promise<Loaded> {
  ...
  const [rdapResult, whois] = await Promise.all([
    lookupRdap(domain),
  ...
  return {
    ...
    rdapUnsupported:
      rdapResult.kind === "unsupported" || rdapResult.kind === "not-found",
    whois,
    base,
    tld,
  };
...
const WhoisPage: FC<PageProps<"/lookup/[domain]/whois">> = async ({ params }) => {
  ...
  return (
    <WhoisTabs
      ...
      whoisPanel={
        data.whois ? <WhoisPanel data={data.whois} base={data.base} /> : null
      }
    />
  );
};
...
const RegistryNotice: FC<{
  ...
  <div className="bg-card border-border/60 mx-auto max-w-xl rounded-2xl border p-8 text-center shadow-sm">
    ...
    <p className="text-muted-foreground mt-2 text-sm">
      SWITCH, the registry for {`.${tld}`} domains, blocks automated WHOIS and
      RDAP queries. Try a manual lookup at{" "}
      <Link
        href={registryUrl}
        target="_blank"
        rel="noreferrer noopener"
...
const WhoisPanel: FC<{ data: WhoisRawByServer; base: string }> = ({
  data,
  base,
}) => (

...
export async function insertRows({
	...
	if (!response.ok) {
		...
		throw new Error(`BigQuery insert failed: ${response.status} ${errorBody}`);
	}
}
...
export async function query({
	...
	return json.rows.map(row =>
		Object.fromEntries(row.f.map((field, index) => [json.schema!.fields[index].name, field.v])),
	);
}
...
export async function getTopDomains(count: number): Promise<string[]> {
	...
	return rows.map(row => row.baseDomain).filter((d): d is string => typeof d === 'string');
}

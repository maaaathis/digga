...
export async function lookupWhoisRaw(
  domain: string,
): Promise<WhoisRawByServer | null> {
...
export async function lookupWhoisParsed(
  domain: string,
): Promise<DomainWhois | null> {
  const base = getBaseDomain(domain);
  if (!base || !isValidLookupDomain(base)) return null;
...
export async function isDomainAvailable(input: string): Promise<boolean> {
  ...
  if (statusArr) {
    ...
    if (
      flat.includes("available") ||
      flat.includes("free") ||
      flat.includes("no object found")
    )
      return true;
  }

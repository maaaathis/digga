...
export async function resolveRecordType(
  ...
  const url = `${endpointFor(resolver)}?name=${encodeURIComponent(
    domain,
  )}&type=${type}`;
  ...
  try {
    response = await fetch(url, {
      ...
      headers: {
        Accept:
          resolver === "cloudflare" ? "application/dns-json" : "application/json",
      },
  ...
  if (!response.ok) {
    console.warn(
      `DoH ${resolver} returned ${response.status} for ${domain} ${type}`,
    );
    return [];
  }
  ...
  return json.Answer.filter(
    (answer) =>
      answer.type in RECORD_TYPE_BY_DECIMAL &&
      RECORD_TYPE_BY_DECIMAL[answer.type] === type,
  ).map((answer) => ({
    name: answer.name,
    type,
    TTL: answer.TTL,
...
export async function resolveAllRecords(
  ...
  const results = await Promise.all(
    RECORD_TYPES.map((type) =>
      resolveRecordType(resolver, domain, type, options),
    ),
  );

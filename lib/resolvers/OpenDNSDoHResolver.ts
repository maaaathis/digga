...
export default class OpenDNSDoHResolver extends DnsResolver {
  public async resolveRecordType(
    domain: string,
    type: RecordType
  ): Promise<RawRecord[]> {
    const response = await fetch(
      `https://doh.opendns.com/dns-query?name=${domain}&type=${type}`,
      {
        method: 'GET',
    ...
    if (!response.ok)
      throw new Error(`Bad response from OpenDNS: ${response.statusText}`);
